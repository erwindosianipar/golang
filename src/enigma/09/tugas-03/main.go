...
func attack() {
	...
	if heros[attacker] == heros[toAttack] {
		fmt.Println("Info: hero tidak bisa menyerang diri sendiri")
	} else {
		if heros[attacker].hp == 0 {
			fmt.Println("Info: hero penyerang sudah mati, tidak dapat melakukan serangan")
		} else {
			if heros[toAttack].hp == 0 {
				fmt.Println("Info: hero yang ingin diserang sudah mati")
			} else {
...
func healing() {
	...
	if heros[healer] == heros[toHeal] {
		if heros[healer].hp == 0 {
			fmt.Println("Info: hero ini sudah mati, tidak bisa melakukan healing")
		} else {
		...
		if heros[healer].hp == 0 {
			fmt.Printf("Info: hero %v sudah mati, tidak bisa melakukan healing pada hero %v", healer, toHeal)
		} else {
			if heros[toHeal].hp == 0 {
				fmt.Println("Info: hero tujuan healing sudah mati, tidak dapat melakukan healing")
			} else {
				healHero(healer, toHeal)
				viewHeroHealed(healer, toHeal)
				main()
